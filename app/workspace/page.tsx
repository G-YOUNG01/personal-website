...
export default async function AdminPage() {
  ...
  const stats = [
    { label: "博客文章", value: postCount?.count || 0, icon: "📝", href: "/workspace/posts" },
    {
      label: "时间线事件",
      value: timelineCount?.count || 0,
      icon: "📅",
    ...
    <div>
      ...
      <div className="card p-6 mb-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <Link href="/workspace/posts/new" className="btn-primary text-center text-sm">
            + 新建文章
          </Link>
          <Link href="/workspace/timelines/new" className="btn-outline text-center text-sm">
            + 添加时间线
          </Link>
          <Link href="/workspace/profile" className="btn-outline text-center text-sm">
            编辑简介
          </Link>
      ...
      <div className="card p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link href="/workspace/posts" className="text-sm text-primary-light hover:underline">
            全部文章 →
          </Link>
        </div>
          ...
          <ul className="divide-y divide-slate-100 dark:divide-white/10">
            {recentPosts.map((p) => (
              <li key={p.id} className="py-3 flex items-center gap-3">
                ...
                <Link
                  href={`/workspace/posts/${p.id}/edit`}
