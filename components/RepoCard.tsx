...
export default function RepoCard({ repo, index }: RepoCardProps) {
  ...
  return (
    ...
    >
      {/* 外链图标：独立兄弟元素（绝对定位右上角），避免 <a> 嵌套 */}
      <a
        href={repo.html_url}
        target="_blank"
        rel="noopener noreferrer"
        aria-label={`${repo.name} 在 GitHub 上查看`}
        className="absolute top-4 right-4 z-10 p-1 rounded text-muted hover:text-foreground hover:bg-primary/10 transition-colors"
      >
      ...
      <Link
        href={`/works/${repo.name}`}
        className="glow-border rounded-xl p-5 block h-full group relative"
      >
        <div className="flex items-start justify-between mb-3">
          <h3 className="text-lg font-semibold text-foreground group-hover:text-primary-light transition-colors pr-6">
            {repo.name}
          </h3>
        </div>
